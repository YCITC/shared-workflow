...
def update_python_init(version):
    ...
    print(f"📝 Updating {PYTHON_INIT.relative_to(PROJECT_ROOT)}...")

    # Create file if it doesn't exist
    if not PYTHON_INIT.exists():
        PYTHON_INIT.parent.mkdir(parents=True, exist_ok=True)
        content = f'"""Python Source"""\n\n__version__ = "{version}"\n'
        PYTHON_INIT.write_text(content)
        print(f"   ✅ Created with version {version}")
        return
...
def update_package_json(version):
    ...
    print(f"📝 Updating {FRONTEND_PACKAGE_JSON.relative_to(PROJECT_ROOT)}...")

    if not FRONTEND_PACKAGE_JSON.exists():
        print(f"   ❌ ERROR: package.json not found")
        sys.exit(1)
...
def generate_version_js(version):
    ...
    print(f"📝 Generating {FRONTEND_VERSION_JS.relative_to(PROJECT_ROOT)}...")
...
'''
    ...
    print(f"   ✅ Generated: v{version} ({git_hash}) on {git_branch}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Version Synchronization Script'
    )
    parser.add_argument(
        '--validate-only',
        action='store_true',
